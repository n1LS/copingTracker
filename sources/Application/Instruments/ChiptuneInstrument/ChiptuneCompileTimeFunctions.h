...
template <size_t... Is> consteval auto gen_semi_lut(std::index_sequence<Is...>) {
...
template <size_t... Is> consteval auto gen_frq_lut(std::index_sequence<Is...>) {
  return std::array<int32_t, sizeof...(Is)>{calculateFrequency(static_cast<int>(Is) - 12)...};
}
...
template <size_t... Is> consteval auto gen_attack_lut(std::index_sequence<Is...>) {
  return std::array<uint16_t, sizeof...(Is)>{calculate_attack_coeff(Is)...};
}

template <size_t... Is> consteval auto gen_decay_lut(std::index_sequence<Is...>) {
