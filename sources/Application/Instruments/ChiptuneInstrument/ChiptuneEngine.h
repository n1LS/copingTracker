...
// issues
static_assert(sizeof(InstrumentParameters) <= 12, "Check sizeof(InstrumentParameters) in error message");
...
typedef struct voice_t {
  ...
  struct pan {
    ...
    inline void tick() {
      ...
      if ((step > 0 && position >= target) || (step < 0 && position <= target)) {
        position = target;
        step = 0;
      }
    }
  ...
  inline void tick_100Hz() {
    ...
    frequency = (int32_t)(((int64_t)frequency * legato.factor) >> 16);
  }
  ...
  inline void tick_1000Hz() {
    ...
    if (timeToLive == 0) {
      ...
      if (burstTime > 0) {
        burstTime--;
        wave = waveNoiseWhite;
        flags.burstEnd = (burstTime == 0);
      } else if (flags.burstEnd) {
  ...
  void command_init_legato(uint8_t speed, int8_t semitones) {
    ...
    if (semitones == 0) {
      ...
      legato.factor = (int32_t)initialFactor; // start at the ratio
      // get total ratio from table (Q16.16)
