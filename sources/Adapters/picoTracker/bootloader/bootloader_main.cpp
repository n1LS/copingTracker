...
extern int parse_uf2_and_write_to_flash(const char *filename, uint32_t target_slot, const char *derived_output_path,
                                        bool do_flash);
...
static int scan_firmware_bins(Uf2FileEntry *entries, int capacity) {
  ...
  while (entry.openNext(&cwd, O_RDONLY) && count < capacity) {
    ...
    if (!entry.isDirectory() && is_bin) {
      bl_copy_str(entries[count].path, sizeof(entries[count].path), FIRMWARE_DIR);
...
static void import_uf2_to_firmwares(const Uf2FileEntry *inbox, int inbox_count) {
  ...
  for (int i = 0; i < inbox_count; ++i) {
    ...
    const int rc = parse_uf2_and_write_to_flash(inbox[i].path, APP_SLOT_ADDR, bin_path, false);
    if (rc != 0) {
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    if constexpr (kEnableAutoBoot) {
      ...
      if (auto_boot_armed && static_cast<int32_t>(now_ms - auto_boot_deadline) >= 0) {
        ...
        if (!boot_firmware_slot(APP_SLOT_ADDR)) {
          menu_show_message("App-slot boot failed. Check flashed firmware image.");
        }
      }
    }
    ...
    if (pressed & KEY_START) {
      if (uf2_count <= 0) {
        menu_show_message("No firmware in /firmwares. Add a UF2 to SD root, reboot.");
      } else {
        ...
        const int rc = parse_uf2_and_write_to_flash(source_uf2, APP_SLOT_ADDR, bin_path, true);
        if (rc == 0) {
          if (!write_firmware_info(source_uf2, bin_path)) {
            menu_show_message("Warning: could not persist firmware_info metadata.");
          }
    ...
    if (display_dirty) {
      menu_render_main(uf2_files, uf2_count, selected_file, installed_firmware, sd_ready, auto_boot_timeout);
      display_dirty = false;
