...
bool boot_firmware_slot(uint32_t slot_base_address) {
  ...
  uint32_t reset_handler = *reinterpret_cast<const uint32_t *>(vector_table + 4u);

  if (!is_valid_stack_ptr(stack_ptr) || !is_valid_reset_handler(reset_handler, slot_base_address)) {
    const uint32_t vector_table_mid = slot_base_address + kVectorTableOffsetMid;
    const uint32_t mid_stack_ptr = *reinterpret_cast<const uint32_t *>(vector_table_mid + 0u);
    const uint32_t mid_reset_handler = *reinterpret_cast<const uint32_t *>(vector_table_mid + 4u);

    if (is_valid_stack_ptr(mid_stack_ptr) && is_valid_reset_handler(mid_reset_handler, slot_base_address)) {
  ...
  if (!is_valid_stack_ptr(stack_ptr) || !is_valid_reset_handler(reset_handler, slot_base_address)) {
    ...
    const uint32_t alt_stack_ptr = *reinterpret_cast<const uint32_t *>(vector_table_alt + 0u);
    const uint32_t alt_reset_handler = *reinterpret_cast<const uint32_t *>(vector_table_alt + 4u);

    if (is_valid_stack_ptr(alt_stack_ptr) && is_valid_reset_handler(alt_reset_handler, slot_base_address)) {
      bootlog("BOOT: using vector table fallback @0x%08x\n", vector_table_alt);
  ...
  const uint32_t reset_handler_addr = reset_handler & ~1u;
  if (reset_handler_addr < slot_base_address || reset_handler_addr >= (slot_base_address + kSlotSize)) {
  ...
  if ((vector_table & 0xFFu) != 0u) {
    bootlog("BOOT: vector table 0x%08x is not 256-byte aligned; refusing handoff\n", vector_table);
    return false;
  }
  const uint32_t aligned_vtor = vector_table;
  bootlog("BOOT: VTOR=0x%08x MSP=0x%08x PC=0x%08x\n", aligned_vtor, stack_ptr, reset_handler);
