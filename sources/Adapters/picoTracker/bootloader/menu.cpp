...
void menu_render_static(void) {
  ...
  render_text(1, 28, "   EDIT " char_border_single_vertical_s " update picoTracker firmware");

  chargfx_draw_changed();
}

void menu_render_main(const Uf2FileEntry *uf2_files, int uf2_count, int selected_index, const char *installed_firmware,
  ...
  if (!sd_ready || uf2_count == 0) {
    menu_show_message(sd_ready ? "No firmware found in /firmwares" : "SD card not mounted");
  } else {
