...
uint32_t msd_sd_get_fat_sectors(void) {
  ...
  if (buf[510] == 0x55 && buf[511] == 0xAA && buf[450] != 0x00) {
    ...
    part_start =
        (uint32_t)buf[454] | ((uint32_t)buf[455] << 8) | ((uint32_t)buf[456] << 16) | ((uint32_t)buf[457] << 24);
  }
  ...
  if (memcmp(buf + 3, "EXFAT   ", 8) == 0) {
    ...
    uint32_t cluster_count =
        (uint32_t)buf[76] | ((uint32_t)buf[77] << 8) | ((uint32_t)buf[78] << 16) | ((uint32_t)buf[79] << 24);
    return (cluster_count * 4 + 511) / 512;
  }

  // FAT32 BPB: sectors per FAT at offset 36 (4 bytes)
  uint32_t fat_size =
      (uint32_t)buf[36] | ((uint32_t)buf[37] << 8) | ((uint32_t)buf[38] << 16) | ((uint32_t)buf[39] << 24);
