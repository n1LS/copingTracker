...
#define SRC_MAX_RATIO 6 // 7350Hz min and 264600Hz max
#define SRC_MAX_RATIO_STR "6"
...
enum SRC_MODE { SRC_MODE_PROCESS = 0, SRC_MODE_CALLBACK = 1 };
...
static inline int psf_lrintf(float x) { return lrintf(x); } /* psf_lrintf */

static inline int psf_lrint(double x) { return lrint(x); } /* psf_lrint */
#endif
